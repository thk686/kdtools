...
When working with a data frame, you can specify which columns to use and the order of inclusion using the ```cols``` argument. Ommitting the ```cols``` arguemnt uses all columns in order.
...
Currently, only a range query is supported.
