...
void print_range_ptr(const pointers_type& x)
{
  auto n = x.size();
  if (n < 10) stop("Not enough rows");
  for (int i = 0; i != 5; ++i)
...
void sort_pointers(NumericVector c1, CharacterVector c2)
{
  ...
  pointers_type idx1(data.size()), idx2(data.size());
  
  auto get_ptr = [](key_type& x){ return &x; };
  transform(begin(data), end(data), begin(idx1), get_ptr);
  transform(begin(data), end(data), begin(idx2), get_ptr);
  
  kd_sort(begin(idx1), end(idx1));
  kd_sort(begin(idx2), end(idx2), greater_key());
  
  Rcout << "Original order\n\n"; print_range(data);
  Rcout << "\nOrder by less\n\n"; print_range_ptr(idx1);
  Rcout << "\nOrder by greater\n\n"; print_range_ptr(idx2);
}
```
...
```{r}
sort_pointers(signif(numbers, 4), strings)
```
